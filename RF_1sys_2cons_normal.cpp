...
double generate_Bernoulli(int numConstraint, int case_index) {
    ...
    for (int i = 0; i < NumSys; i++) {
        ...
        for (int j = 0; j < numConstraint; j++) {
            ...
            x_value[j] = p[j] + correlated_normal[j] * std::sqrt(vari[j]);

            // Ensure the values are within the interval [0, 1]
            if (x_value[j] < 0) {
                x_value[j] = 0;
            }
            else if (x_value[j] > 1) {
            ...
            // If you want generate real bernoulli data...

...
double configuration(void) {
    ...
    for (int j = 0; j < NumConstraint; j++) {
        UB = maxthreshold*theta[j]/(maxthreshold*theta[j] + (1-maxthreshold));
        LB = maxthreshold/(maxthreshold + (1-maxthreshold)*theta[j]);
        epsilon[j] = (UB-LB)/2;
        //printf("epsilon: %.4f\n", epsilon[j]);
    }

...
#define DOMBERF 0   //If you do mberf, then this is 1. Or this is 0 (Only BeRF is implemented.)
...
// 이 함수는 berf()가 완료된 후 호출되어야 합니다.
// 각 시스템 i에 대해, constraint 0의 threshold d0와 constraint 1의 threshold d1에서
// 모두 feasible (즉, BeRF_Z[i][j][d] == 1)로 결정된 경우를 카운트합니다.
void printFeasibleTable() {
    // 테이블 크기는 NumThreshold x NumThreshold (여기서는 4x4)
    int table[NumThreshold][NumThreshold] = {0};

    // 각 시스템에 대해 두 constraint의 각 threshold 결정값을 조사합니다.
    for (int i = 0; i < NumSys; i++) {
        for (int d0 = 0; d0 < NumThreshold; d0++) {
            for (int d1 = 0; d1 < NumThreshold; d1++) {
                // constraint 0의 threshold d0와 constraint 1의 threshold d1에서
                // 모두 1이면 (feasible라고 판단) 해당 셀에 카운트
                if (BeRF_Z[i][0][d0] == 1 && BeRF_Z[i][1][d1] == 1) {
                    table[d0][d1]++;
                }
            }
        }
    ...
    // 인덱스까지 포함한 표를 출력 (5x5)
    // 첫 행에 열의 threshold index 표시
    std::cout << std::setw(8) << " ";
    for (int col = 0; col < NumThreshold; col++) {
        std::cout << std::setw(8) << ("Thr" + std::to_string(col));
    }
    std::cout << "\n";

    // 각 행: 왼쪽에 해당 행의 threshold index, 이후 각 셀의 count 출력
    for (int row = 0; row < NumThreshold; row++) {
        std::cout << std::setw(8) << ("Thr" + std::to_string(row));
        for (int col = 0; col < NumThreshold; col++) {
